...
public class GvrReticlePointerJomo : GvrBasePointer
{
    [Range(0,1)]
    public float circlePercent = 0;
    ...
    {
        targetObj = raycastResultResult.gameObject;
        if (targetObj != null)
        {
            StartReticle(0.002f);
    ...
    public override void OnPointerHover(RaycastResult raycastResultResult, bool isInteractive)
    {
        SetPointerTarget(raycastResultResult.worldPosition, isInteractive);
        //print(circlePercent);
        if (circlePercent > 0.99f)
        {
            EventTrigger evtTrigger = raycastResultResult.gameObject.GetComponent<EventTrigger>();
            if (evtTrigger != null)
            {
                //targetObj = raycastResultResult.gameObject;
                if (targetObj != null)
                {
                    // Invoke Click here
                    ExecuteEvents.Execute<IPointerClickHandler>(targetObj,
    ...
    {
        UpdateDiameters();
        circlePercent = Mathf.Clamp01(circlePercent + percentSpeed);
        MaterialComp.SetFloat("_Ratio", circlePercent);
    }
    ...
    private void CreateReticleVertices2()
    {
        Mesh mesh = new Mesh();
        gameObject.AddComponent<MeshFilter>();
        GetComponent<MeshFilter>().mesh = mesh;
        ...
        for (int si = 0; si <= segments_count; ++si)
        {
            ...
            vertices[vi] = new Vector3(x, y, 0.0f); // Outer vertex.
            uvs[vi++] = new Vector2( ((float)si / (float)segments_count), 0) ;
            vertices[vi] = new Vector3(x, y, 1.0f); // Inner vertex.
            uvs[vi++] = new Vector2(((float)si / (float)segments_count), 0);
        }
        #endregion
    ...
    public void StartReticle(float speed) {
        percentSpeed = speed;
    }
